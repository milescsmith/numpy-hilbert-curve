...
from hilbert.gray import gray2binary

SIXTYFOURBITS: Final[int] = 64


def encode(locs: NDArray, num_dims: int, num_bits: int) -> NDArray:
    ...
    if orig_shape[-1] != num_dims:
        msg = f"The shape of locs was surprising in that the last dimension was of size {orig_shape[-1]}, but num_dims={num_dims}.  These need to be equal."
        raise ValueError(msg)

    if num_dims * num_bits > SIXTYFOURBITS:
        msg = f"num_dims={num_dims} and num_bits={num_bits} for {num_dims * num_bits} bits total, which can't be encoded into a uint64.  Are you sure you need that many points on your Hilbert curve?"
        raise ValueError(msg)
    ...
    # Now flatten out.
    gray = np.reshape(np.swapaxes(gray, axis1=1, axis2=2), (-1, num_bits * num_dims))
    ...
    padded = np.pad(hh_bin, ((0, 0), (extra_dims, 0)), mode="constant", constant_values=0)

    # Convert binary values into uint8s.
    hh_uint8 = np.squeeze(np.packbits(np.reshape(padded[:, ::-1], (-1, 8, 8)), bitorder="little", axis=2))
